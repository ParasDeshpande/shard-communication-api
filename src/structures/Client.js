...
class Client extends events_1.EventEmitter {
    ...
    constructor(options = {}) {
        ...
        if (typeof this.options.host !== 'string')
            throw new TypeError("Client option 'host' must be a 'string'.");
        if (typeof this.options.port !== 'number')
            throw new TypeError("Client option 'port' must be a 'number'.");
        if (typeof this.options.password !== 'string')
            throw new TypeError("Client option 'password' must be a 'string'.");
        if (typeof this.options.retryAmount !== 'number')
            throw new TypeError("Client option 'retryAmount' must be a 'number'.");
        if (typeof this.options.retryDelay !== 'number')
            throw new TypeError("Client option 'retryDelay' must be a 'number'.");
        if (typeof this.options.secure !== 'boolean')
            throw new TypeError("Client option 'secure' must be a 'boolean'.");
        if (typeof this.options.clientid !== 'string')
            throw new TypeError("Client option 'clientid' must be a 'string'.");
        if (typeof this.options.shardid !== 'string')
            throw new TypeError("Client option 'shardid' must be a 'string'.");
        this.socket = null;
    }
    /**
     ...
     * Client methods
     */
    send(recieverFilter, d) {
        return new Promise((resolve, reject) => {
            if (!this.socket)
                return reject("Not connected.");
            if (!recieverFilter)
                return reject("No reciever filter provided.");
            if (!recieverFilter.clientid || Array.isArray(recieverFilter.clientid))
                return reject("'clientid' in reciever filter must be a string array.");
            if (recieverFilter.shardid && !Array.isArray(recieverFilter.shardid))
                return reject("'shardid' in reciever filter must be a string array.");
            if (!d || !JSON.stringify(d).startsWith("{"))
